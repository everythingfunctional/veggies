module veggies
    use veggies_assert_m, only: &
            ...
            assert_not, &
            assert_that
    use veggies_ascii_string_generator_m, only: ASCII_STRING_GENERATOR
    use veggies_double_precision_input_m, only: double_precision_input_t
    ...
    use veggies_integer_generator_m, only: INTEGER_GENERATOR
    ...
    use veggies_run_tests_m, only: run_tests
    use veggies_shrink_result_m, only: &
            shrink_result_t, shrunk_value, simplest_value
    use veggies_string_input_m, only: string_input_t
    use veggies_test_m, only: filter_result_t, test_t
    use veggies_test_case_result_m, only: &
            test_case_result_t
    use veggies_test_collection_result_m, only: &
            test_collection_result_t
    use veggies_test_constructors_m, only: &
