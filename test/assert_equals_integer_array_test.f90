module assert_equals_integer_array_test
    ...
    implicit none
    private
    public :: test_assert_eq_integer_arr
...
contains
    function test_assert_eq_integer_arr() result(tests)
        type(test_item_t) :: tests

        tests = describe( &
                "assert_equals with integer arrays", &
                [ it( &
                        "passes with the same array", &
                        check_pass_for_same_array) &
                , it( &
                        "fails for arrays with different numbers", &
                        check_fail_for_different_numbers) &
                , it( &
                        "fails for arrays with different sizes", &
    ...
    pure function check_fail_for_different_numbers() result(result_)
        ...
        associate(example1 => [1, 2], example2 => [3, 4])
    ...
    pure function check_fail_for_different_sizes() result(result_)
        ...
        associate(example1 => [1, 2], example2 => [1, 2, 3])
            example_result = assert_equals(example1, example2)
            example_result_c = assert_equals(example1, example2, BOTH_MESSAGE)
            example_result_s = assert_equals(example1, example2, var_str(BOTH_MESSAGE))
            example_result_cc = assert_equals( &
